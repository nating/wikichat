    ...
    <section className="flex flex-col gap-3">
      <ul className="space-y-2">
        {urls.map((url) => (
          <li
            key={url}
            className="flex justify-between items-center bg-background hover:bg-brand/10 border border-gray-300 dark:border-gray-700 rounded-xl px-4 py-3 text-sm text-foreground shadow-sm transition-all"
          >
            <span className="break-all flex-1 mr-4 font-sans">{url}</span>
            <button
              onClick={() => setModalUrl(url)}
              disabled={loadingDelete === url}
              className="text-xs font-medium text-red-600 hover:text-red-700 transition font-sans"
            >
              {loadingDelete === url ? 'Deleting...' : 'Delete'}
            </button>
          </li>
        ))}
      </ul>
